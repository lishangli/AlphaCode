...
from alphacode.core.tree import SearchTree
...
class MCTSController:
    ...
    def __init__(self, config: MCTSConfig):
        ...
        # Search state
        self.search_tree: SearchTree | None = None
        self.session_id: str = ""
    ...
    def _run_iteration(self):
        ...
        def get_confidence(a):
            return a.confidence if hasattr(a, 'confidence') and a.confidence else 0.5
        actions.sort(key=get_confidence, reverse=True)
    ...
    def _select_node(self) -> MCTSNode | None:
...
Respond with just a number (e.g., "0.9" or "0.3")."""
            ...
            try:
                ...
                if match:
                    ...
                    logger.info(
                        f"Self-assessment: "
                        f"confidence={self_confidence:.3f}, entropy={entropy:.3f}"
                    )
                    ...
                    if self_confidence > 0.8 and entropy < 0.05:
                        num_branches = 1
                        logger.info("High confidence detected, using single branch")
                    else:
                        num_branches = 2
    ...
    def _try_action(self, parent: MCTSNode, action: Action) -> MCTSNode | None:
    ...
    def _quick_relevance(self, code: str, goal: str) -> float:
        ...
        total_score = 0
        for _concept, keywords in matched_concepts:
            for kw in keywords:
                if kw in code_lower:
                    total_score += 1
